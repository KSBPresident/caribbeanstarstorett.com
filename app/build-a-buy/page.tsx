...
const categoryLabels: Record<string,string> = { products:"Products",services:"Services",businesses:"Businesses",jobs:"Jobs","real-estate":"Real Estate","multi-item":"Build-A-Buy bundle" };
...
export default async function BuildABuyPage({searchParams}:PageProps) {
  ...
  const {data:requests}=await supabase.from("purchase_requests").select("id,title,details,category_key,budget_amount,currency,status,created_at").eq("requester_user_id",user.id).order("created_at",{ascending:false}).limit(50);
  const message=params.notice==="created"?"Your request has been saved to your account.":params.notice==="cancelled"?"Your open request was cancelled.":params.error==="invalid"?"Check the request title, details, category, and budget.":params.error==="cancel"?"This request could not be cancelled. It may already be closed.":params.error==="save"?"We couldn’t save your request. Please try again.":null;
  return <><SiteHeader/><main className="identity-page">
    <header className="identity-heading"><div><span className="identity-eyebrow">CARIBBEAN STAR STORE · BUILD-A-BUY</span><h1>Tell us what you need</h1><p>Describe a product, service, or combination and save the request to your account.</p></div></header>
    ...
    <div className="build-request-grid">
      <section className="identity-panel"><h2>Create a request</h2><p>Requests are private to your account. Seller matching and fulfillment are not connected yet.</p>
        ...
        </form><p className="identity-note">Your saved request is visible only to your account until the matching workflow is connected.</p>
      </section>
      <section className="identity-panel"><div className="workspace-section-heading"><div><span className="identity-eyebrow">YOUR ACTIVITY</span><h2>Your requests</h2></div><span className="workspace-count">{requests?.length||0} saved</span></div>
        {requests?.length?<div className="purchase-request-list">{requests.map(request=><article className="purchase-request-card" key={request.id}>
        ...
        </article>)}</div>:<p className="organization-empty">You haven’t saved a request yet.</p>}
