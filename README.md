...
- WooCommerce catalog access remains blocked by a Cloudflare challenge. The connected WPVibe diagnostic reported a challenge for `/wp-json/wpvibe/v1/site-info`, and a read-only fetch of the public `/shop/` page also received a challenge. These responses do not identify who manages the protection or which rule matched. The marketplace falls back to a link to the existing store; live product browsing through this app remains blocked.
...
1. The site owner uses HostGator/cPanel and does not have a Cloudflare account. Run Connection Check in the WPVibe plugin, then give its report to HostGator support and ask them to identify whether a security layer in front of the domain is challenging WPVibe's WordPress API request. Do not change the rule until the responsible provider identifies the cause. Until catalog reads work, keep the existing-store handoff.
