...
- Public getting-started guide for shoppers, buyers, sellers, and business organizations
...
## Current status and launch gates (2026-09-25)
...
- WooCommerce catalog access remains blocked. The connected WPVibe diagnostic for a WordPress API route and a read-only request for the public `/shop/` page received security challenges. The Vercel preview's WooCommerce Store API product request currently returns HTTP 403. These responses do not identify which service or rule is enforcing the block. The site owner uses HostGator/cPanel and does not have a Cloudflare account. The marketplace links to the existing store while public catalog reads remain blocked.
...
- The repository contains thirteen Supabase migration files, matching the connected project's migration ledger. The latest migration adds indexes to the requester and applicant foreign keys used by account activity and seller applications; the connected project's performance advisor no longer reports those two foreign keys as unindexed. The membership-hardening and organization-audit migrations preserve the recorded SQL statements. The two `rls_auto_enable` hardening entries remain conditional because that event-trigger helper is Supabase-environment-specific. Fresh-project replay still needs verification before treating the migration set as ready for a new project.
...
1. The site owner uses HostGator/cPanel and does not have a Cloudflare account. Run Connection Check in the WPVibe plugin, then share its report and the Vercel preview's `/api/health` response with HostGator support. Ask support to inspect the server security logs for the read-only request to `/wp-json/wc/store/v1/products` from the Vercel preview, identify the service or rule returning 403, and recommend a narrow exception for public product GET requests. Keep write methods and other API routes protected. Until catalog reads work, keep the existing-store handoff.
