...
- The repository now contains twelve Supabase migration files, including the original identity/kernel schema recovered from the connected migration history. The two rls_auto_enable hardening entries are conditional because that event-trigger helper is Supabase-environment-specific. Six later feature migration filenames differ from the recorded versions in the connected project; reconcile those timestamps before applying repository migrations back to that project. Fresh-project replay still needs verification.
