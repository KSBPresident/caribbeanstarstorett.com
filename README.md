...
- The repository contains twelve Supabase migration files. The six feature migration filenames now match the connected project's ledger, and the membership-hardening and organization-audit migrations have been restored to their recorded SQL statements. The two `rls_auto_enable` hardening entries remain conditional because that event-trigger helper is Supabase-environment-specific. Fresh-project replay still needs verification before treating the migration set as ready for a new project.
